...
var Mat = function(__row, __col, __type, __channel, __buffer){
	this.row = __row || error(arguments.callee, IS_UNDEFINED_OR_NULL, 179);
	this.col = __col || error(arguments.callee, IS_UNDEFINED_OR_NULL, 180);
	this.channel = __channel;
	__type ? __type.call(this, __row, __col, __channel, __buffer) : error(arguments.callee, UNSPPORT_DATA_TYPE, 182);
	this.data.length === this.row * this.col * this.channel || error(arguments.callee, MAT_SIZE_ERROR, 183);
};
cv.Mat = Mat;
/***********************************************
 *	<h3>Mat.getType</h3>
...
Mat.prototype.at = function(__type, __x, __y){
	var type = __type || error(arguments.callee, IS_UNDEFINED_OR_NULL, 392);
	var x = __x || 0,
		y = __y || 0,
	...
	switch(type){
		...
		case "doulble":
			...
			break;
		default:
			error(arguments.callee, UNSPPORT_DATA_TYPE, 437);
	}
...
var Rect = function(__array, __channel){
	this.data = __array || [];
	this.channel = __channel || error(arguments.callee, IS_UNDEFINED_OR_NULL, 453);
};
cv.Rect = Rect;
...
var imread = function(__image){
	__image || error(arguments.callee, IS_UNDEFINED_OR_NULL, 466);
 ...
 *	in - The input Mat.
 */
var imwrite = function(__imgMat){
	__imgMat || error(arguments.callee, IS_UNDEFINED_OR_NULL, 488);
	...
	if(__imgMat.type === "CV_RGBA"){
		...
		error(arguments.callee, UNSPPORT_DATA_TYPE, 504);
		return false;
	}
};
cv.imwrite = imwrite;
...
function convertScaleAbs(__src, __dst){
	__src || error(arguments.callee, IS_UNDEFINED_OR_NULL, 520);
...
var addWeighted = function(__src1, __alpha, __src2, __beta, __gamma, __dst){
	(__src1 && __src2) || error(arguments.callee, IS_UNDEFINED_OR_NULL, 565);
	...
	if(height !== __src2.row || width !== __src2.col || channel !== __src2.channel){
		error(arguments.callee, "Src2 must be the same size and channel number as src1!", 573);
		return null;
	}
...
var cvtColor = function(__src, __code){
	__src || error(arguments.callee, IS_UNDEFINED_OR_NULL, 615);
	if(__src.type && __src.type.indexOf("CV_") > -1){
		...
		error(arguments.callee, UNSPPORT_DATA_TYPE, 642);
	}
	return dst;
};
...
var brightnessContrast = function(__src, __brightness, __contrast){
	__src || error(arguments.callee, IS_UNDEFINED_OR_NULL, 659);
	if(__src.type === "CV_RGBA"){
		...
		error(arguments.callee, UNSPPORT_DATA_TYPE, 707);
	}
	return dst;
};
	...
	if(__p < 0 || __p >= __len){
		switch(__borderType){
			...
			case CV_BORDER_CONSTANT:
				__p = -1;
			default:
				error(arguments.callee, UNSPPORT_BORDER_TYPE, 759);
		}
	}
	return __p;
};
...
var remap = function(__src, __mapX, __mapY, __dst){
	(__src && __mapX && __mapY) || error(arguments.callee, IS_UNDEFINED_OR_NULL, 1493);
	
	if(__src.type !== "CV_RGBA")
		error(arguments.callee, UNSPPORT_DATA_TYPE, 1496);
	...
	if(typeof __mapX === "function" && typeof __mapY === "function"){
		...
		error(arguments.callee, UNSPPORT_DATA_TYPE, 1503);
		return __dst || __src;
	}
};
cv.remap = remap;
...
var warpAffine = function(__src, __rotArray, __dst){
	...
	if(__src.type && __src.type === "CV_RGBA"){
		...
		error(arguments.callee, UNSPPORT_DATA_TYPE, 1606);
	}
	return dst;
};
